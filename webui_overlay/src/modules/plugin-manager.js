...
class PluginManager {
  ...
  async triggerHook(hookName, data = {}) {
    ...
    if (window.core.isDebugMode()) {
      window.core.logDebug(`Hook ${hookName} triggered with data:`, data);
    }
  }
  ...
  refreshButtons() {
    // 刷新顶栏按钮
    this.refreshButtonsInLocation('header');
    
    // 刷新侧栏按钮
    this.refreshButtonsInLocation('sidebar');
    
    // 刷新底栏按钮
    this.refreshButtonsInLocation('bottom');
  }

  refreshButtonsInLocation(location) {
    const container = document.getElementById(`plugin-${location}-actions`);
    if (!container) return;
    ...
    // 添加插件按钮
    this.pluginButtons[location].forEach(button => {
      const buttonElement = this.createButtonElement(button, location);
  ...
  createButtonElement(button, type = 'header') {
    ...
    buttonElement.className = `plugin-${type}-btn`;
    buttonElement.title = button.title || '';
    buttonElement.innerHTML = `<span class="material-symbols-rounded">${button.icon}</span>`;
